...
Large File Split & Join (chunks to 6 GB, streaming shrink, no-load inspect)
...
| `npm test` | vitest unit suite (78 tests: search scoring, guides library, ranges, CSV, engine round-trips, sign/annotate/crop/forms/slides, merge-select, split modes, metadata, markdown, image extraction, progress/recents helpers) |
...
- **Real URLs, not hash routes:** every tool lives at `/merge-pdf/` etc.,
  plus a `/guides/` hub with 15 long-tail tutorials (History API + click
